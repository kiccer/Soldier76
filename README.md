...
### 安装教程
1. **以管理员身份**启动罗技驱动 (请确保你下载的驱动与你的操作系统是匹配的)
2. 扫描游戏 (如果自动检测无法找到游戏，则需要手动添加)
3. 开启 **自动游戏检测** (这样做比较好，以免脚本意外干扰正常操作)
4. 右键配置文件右侧 **PUBG** 图标 -> **编写脚本**
5. 将 **Soldier76.lua** 中包含的所有代码完全覆盖进去，保存(Ctrl + S)
6. 右键 **PUBG配置** 和 **默认配置** 中的 **G6**, **G7**, **G8**, **G9**, **G10**, **G11** 按钮 -> **取消分配**

### 游戏设置
* 修改 `userInfo.AimingSettings` 为 `default` ，你就可以不需要改任何设置。
  - 使用该设置，则你的任何点击操作都会被识别为射击，会触发自动压枪
  - 如果你只是单纯的想要进行点击操作，则需按住左shift或者左alt
* 脚本默认使用 `recommend` 设置，如果使用推荐设置，则需要对游戏内设置做相应改动
  - 开镜：长按右键
  - 瞄准：长按左ctrl
* 如果你的操作习惯比较与众不同，也可以使用 `custom` 设置
  - 使用该设置需要在 `pubg.isAimingState` 方法中做对应的判断
  - 建议对该脚本有一定认识后再使用，或者请加入交流群获取帮助
* 如果要使用自动连发功能，则需把开火键改为键盘按键（例如：~键），可选，不推荐
